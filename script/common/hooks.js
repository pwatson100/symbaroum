...
Hooks.on('renderChatMessage', async (chatItem, html, data) => {
  console.log("ping");
  const flagData = await chatItem.getFlag(game.system.id, 'abilityRoll');
  if(flagData){
    console.log(flagData);
    await html.find("#applyEffect").click(async () =>  {
      if(flagData.tokenId){
        const token = canvas.tokens.objects.children.find(token => token.data._id == flagData.tokenId);
        ...
        if(flagData.removeEffect){
          let statusEffectCounter = EffectCounter.findCounter(token, flagData.removeEffect);
